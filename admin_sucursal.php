...
?>		
	    ...
	    <form method="GET" id="formulario" >
	    	<table>
				...
				<tr>
					<td><a href="add_sucursal.php" class="btn btn-primary" style="margin-top:5%;">Agregar Sucursal</a></td>
				</tr>
			 </table></form>
			</div>
			<script>
				function ver(id,sr)
				{
					...
					{
						location.replace("add_sucursal.php?id="+id);
					}
					else if(select == 2)
					{
						if(window.confirm('¿Desea eliminar esta sucursal'))
						{
							location.replace("admin_sucursal.php?action=elimin&id="+id);
						}
					}
				}
			</script>
<?php
   ...
   if(isset($_GET["q"])){
	$q = $_GET["q"];
	$query = mysql_query("SELECT * FROM sucursal WHERE nombre LIKE '%$q%'");}
	else
	{
	$q="";
	$query = mysql_query("SELECT * FROM sucursal");
	}
